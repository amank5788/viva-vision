...
const reactToSelectedChats = () => {
  ...
  chatInstances.forEach((chatInstance) => {
    ...
    generateResponseBtn.classList.remove("bg-green-600");
    generateResponseBtn.classList.add("bg-red-600");
  });
  ...
  if (startChat !== null && endChat !== null) {
    ...
    generateResponseBtn.classList.remove("bg-red-600");
    generateResponseBtn.classList.add("bg-green-600");
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (
    ...
    chat.forEach((chatInstance) => {
      ...
      chatDiv.innerHTML =`<p style="color: rgb(253 224 71); font-weight: 600">${chatInstance.role}</p>
                        <p>${chatInstance.content}</p>`
      ...
      if (document.querySelector(`[ticId="${chatInstance.ticId}"]`) === null) {
        ...
        existingChatDiv.innerHTML = `<p style="color: rgb(253 224 71); font-weight: 600" >${chatInstance.role}</p>
        <p>${chatInstance.content}</p>`;
      }
    });
  }
  if (request.type === "CHATGPT_RESPONSE") {
    ...
    responseContainer.innerHTML = request.data.generatedResponse;
  }
